...
Cypress.Commands.add('fillBrandBackendSectionOut', (card_id) => {
  ...
  cy.get('#identity').type(`${window.Identity}`).should('have.value', `${window.Identity}`);
  cy.get('#authorization').type(`${window.Authorization}`).should('have.value', `${window.Authorization}`);
...
Cypress.Commands.add('verifyReavealedInfo', (card_num, card_exp, card_cvv) => {
  ...
  cy.get('[name=number]').then($iframe => {
    ...
    cy.wrap($body).find('#result').should('have.text', card_num);
  ...
  cy.get('[name=expiry]').then($iframe => {
    ...
    const $body = $iframe.contents().find('body');
    cy.wrap($body).find('#result').should('have.text', card_exp);
  ...
  cy.get('[name=cvv]').then($iframe => {
    ...
    const $body = $iframe.contents().find('body');
    cy.wrap($body).find('#result').should('have.text', card_cvv);
